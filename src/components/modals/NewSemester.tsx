import { initRooms, initSemester } from "@/api/constants";
...
const NewSemester = ({
  ...
  const onAdd = () => {
    ...
    setList((prevSemList) => [...prevSemList, toAdd]);
    setSemester({
      startYear: "",
      endYear: "",
      sem: "1ST",
  ...
  const saveToDB = async (docId: string) => {
    try {
      await setDoc(doc(db, "semesters", docId), initSemester);
    } catch (error) {
      console.log(error);
    }
  };
